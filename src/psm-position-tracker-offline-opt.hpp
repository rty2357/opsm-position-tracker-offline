...
#ifndef PSM_POSITION_TRACKER_OFFLINE_OPT_HPP_
#define PSM_POSITION_TRACKER_OFFLINE_OPT_HPP_
...
namespace psm {
	namespace position_tracker {
		...
		const struct option LongOpt[] = {
				...
				{ConfIni_LaserScannerLogName.item,	required_argument,	0,	'S'},
				{ConfIni_OdometryLogName.item,		required_argument,	0,	'O'},
		...
		inline bool options::get_option(int aArgc, char **aArgv)
		{
			...
			while(1){
				...
				case 'h':
				{
					...
					fprintf(stderr, "\t\t\x1b[1m%s\x1b[0m - position tracker using probabilistic scan matching\n", proc_name);
					fprintf(stderr, "\n");
					...
					fprintf(stderr, "\t\t\x1b[1m%s\x1b[0m is estimate the robot position and optimize it using probabilistic scan matching.\n", proc_name);
